        ...
        >
          {metrics.map((metric, index) => (
            <motion.div key={index} variants={fadeInUp}>
              <Card className="text-center p-6" hover={false}>
                <div className="text-2xl md:text-3xl font-bold text-white financial-number mb-1">
                  {metric.value}
                </div>
                ...
                <div className="p-6 md:p-8">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                    <div className="flex items-center gap-3">
                      <span className="text-4xl">{deal.commodityIcon}</span>
                      <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
                    <div>
                      ...
                      <div className="text-xl font-bold text-white financial-number">
                    ...
                    {deal.deliveries && (
                      <div>
                        ...
                        <div className="text-xl font-bold text-white flex items-center gap-2">
                          ...
                          {deal.deliveries >= 1000 ? "1000s" : deal.deliveries}
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  {(deal.minOrder || deal.maxOrder) && (
                    <div className="mt-4 pt-4 border-t border-slate-800">
                      <div className="flex flex-wrap gap-4 text-sm text-slate-400">
                        {deal.minOrder && (
                          <span>
                            Min order: {formatCurrency(deal.minOrder)}
                          </span>
                        )}
                        {deal.maxOrder && (
                          <span>
                            Max order: {formatCurrency(deal.maxOrder)}
                          </span>
                        )}
                        {deal.counterparty.established && (
                          <span className="text-emerald-400">
                            Established: {deal.counterparty.established}
                          </span>
                        )}
                      </div>
                    </div>
